public class SlingshotWindow : ElementaryWidgets.CompositedWindow {
    ...
    public SlingshotWindow () {
        ...
        top.pack_start (this.categories, true, true, 15);
        
        this.top_spacer = new Gtk.HBox (false, 0);
        this.top_spacer.realize.connect ( () => { this.top_spacer.visible = false; } );
        this.top_spacer.can_focus = true;
        top.pack_start (this.top_spacer, false, false, 0);
        
        this.searchbar = new Slingshot.Frontend.Searchbar ("Start typing to search...");
        this.searchbar.changed.connect (this.search);
        top.pack_start (this.searchbar, false, true, 15);
